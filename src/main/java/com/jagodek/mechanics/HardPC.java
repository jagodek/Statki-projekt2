...
public class HardPC implements Enemy, DeadObservable {
    ...
    private final Queue<Position> potentialShip = new LinkedList<>();


    public HardPC(Sea mySea, Sea enemySea) {
    ...
    public List<Position> makeHit() {
        Position toHit;
        if (potentialShip.isEmpty()) {
            toHit = randomFromSet();
        } else {
            toHit = potentialShip.poll();
            System.out.println("aaaaaaaaaaaaaaa");
        }
        toBeHit.remove(toHit);
        mySea.hit(toHit);
        if (mySea.stateOfSquare(toHit) == StateOfSquare.HIT) {
            Iterator it = potentialShip.iterator();
            while (it.hasNext()) {
                if (!toBeHit.contains(it.next())) {
                    it.remove();
                }
            }
            Position posTemp;
            for (int i = -1; i < 2; i += 2) {
                ...
                posTemp = new Position(toHit.getX() + i, toHit.getY());
                if (toBeHit.contains(posTemp)) {
                    potentialShip.add(posTemp);
                }
                posTemp = new Position(toHit.getX(), toHit.getY() + i);
                if (toBeHit.contains(posTemp)) {
                    potentialShip.add(posTemp);
                }
